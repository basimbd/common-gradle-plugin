...
abstract class Common implements Plugin<Project> {
    ...
    String findVersionSuffixForFeatureBranch() {
        final String PULL_REQUEST_PROPERTY_NAME = 'pullReqNum'
        final String RUNNING_BRANCH_PROPERTY_NAME = 'runningBranch'
        final String RELEASE_BRANCH_PATTERN = /\d+\.\d+\.[0z]/

        if (project.hasProperty(PULL_REQUEST_PROPERTY_NAME))
            return "-PR${project.property(PULL_REQUEST_PROPERTY_NAME)}"
        else if (project.hasProperty(RUNNING_BRANCH_PROPERTY_NAME)) {
            String runningBranch = project.property(RUNNING_BRANCH_PROPERTY_NAME)
            if (!(runningBranch.contains('origin/master') || runningBranch =~ RELEASE_BRANCH_PATTERN)) {
                def matcher = runningBranch =~ /IDETECT-\d+/
                if (matcher)
                    return '-' + matcher[0]
                else
